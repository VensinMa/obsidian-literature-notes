...
**⚠️ 核心原则：完整翻译，不得节选或精简概括**

**章节结构规范：**

| 规则 | 说明 |
|------|------|
| **灵活结构** | 根据原文实际结构自动调整，不强制固定章节 |
| **层级保持** | 原文的一级标题 → `###`，二级标题 → `####`，三级标题 → `#####` |
| **完整性** | 无论原文多长，均需完整翻译所有内容 |

**常见文章类型结构参考：**

| 文章类型 | 典型章节结构 |
|----------|--------------|
| **原创研究** | Abstract → Introduction → Results → Discussion → Methods |
| **综述** | Abstract → Introduction → [主题章节1] → [主题章节2] → ... → Conclusions/Summary |
| **短报告** | Abstract → Main Text → Methods |
| **方法文章** | Abstract → Introduction → Protocol/Procedure → Applications |
...
| **代码块** | 保留原文代码，注释翻译为中文 |
| **引用文献** | 保留原文引用编号，不翻译 |
| **缩写** | 首次出现时标注全称，如`GWAS（全基因组关联分析）` |
| **专有名词** | 首次出现时标注英文，如`转座子（Transposable Element, TE）` |
